...
for patient_data in patients:
    try:
        ...
        if not User.objects.filter(username=patient_data["username"]).exists():
            ...
            # Step 2: Create the UserProfile instance and set role to "PATIENT"
            UserProfile.objects.create(user=user, role='PATIENT')

            # Step 3: Create the Patient instance
            Patient.objects.create(
                user=user,
                phone=patient_data["phone"],
                address=patient_data["address"],
            ...
            print(f"Patient {patient_data['username']} already exists. Skipping creation.")

    except IntegrityError as e:
        print(f"IntegrityError occurred for {patient_data['username']}: {e}")

print("Patient creation process completed.")